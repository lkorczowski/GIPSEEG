function [Bs Bt As At Class eigV]=CSTPover(X,Y,P,W,PLOT)
...
for k=1:size(X,3)
    Cs_k(:,:,k)=X(:,:,k)*X(:,:,k)'/(size(X,2)-1);
    Ct_k(:,:,k)=X(:,:,k)'*X(:,:,k)/(size(X,1)-1);
end
...
for z=1:nbClass%find(Class==1) %%%%%%%%%%%%%%%%%%%% WARNING
    if Bool==1
    P{z}=mean(Xtmp(:,:,find(Y==Class(z))),3); % WITH NO OVERLAPPING
    end
...
for z=1:length(P)
    ...
    disp('UzT*Z*Vz')
        Uz{z}'*Z{z}*Vz{z}
    %}
    eigV=find(diag(Wz{z}.^2)>SNR);
    
    if max(eigV)>(size(X,1)-1)
    eigV=1:size(X,1)-1;
    end
    if isempty(eigV)
        eigV=1;
