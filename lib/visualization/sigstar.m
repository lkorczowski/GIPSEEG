function varargout=sigstar(groups,stats,nosort,AddBar)
...
for ii=1:length(groups)
	...
	H(ii,:)=makeBar(xlocs(ii,:),thisY,stats(ii),AddBar);
end
%-----------------------------------------------------
...
function H=makeBar(x,y,p,ShowBar)
...
x=repmat(x,2,1);
y=repmat(y,4,1);
H(1)=plot(x(:),y,Color,'LineWidth',1.5);
