function [W,e,niter] = mean_power(COV,i_cfg)
...
% - dual: if we use the regular (dual = 0) or the dual
%         (dual = 1) algorithm.
...
cfg = struct('tol',10^(-3),'N_itermax',100,'W',Wdef,'t',0.1,'phi',0.8);
...
while (niter<cfg.N_itermax)
    ...
    H = 0;
    for indN=1:N
        H = H + (P*COV(:,:,indN)*transpose(P))^(cfg.t);
    end
    H = H/N;
    Ptmp = H^(-cfg.phi)*P;
