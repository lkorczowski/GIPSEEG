...
for indT=1:2
    switch indT
        case 1
            ...
            load('MARTI_GroupsName.mat');ALLdata.subject=GroupsName
        case 2
    ...
    clear Parameters R
%     Parameters.method_mean = {'opttransp','congedo','ld','riemann'}; % PARAMETER ld for logdet alphadivergence (Chebbi & Moakher, 2012)
Parameters.method_mean = {'power','power_dual'}
    Parameters.method_dist = {'riemann','ld'}; %PARAMETER
% Parameters.method_dist = {'ld'};
...
%     Parameters.TestSetRatio={0.1 0.3 0.5};
     Parameters.TestSetRatio={0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.40 0.45 0.5};
    ...
    clear P
    Parameters
    groups2take=1:length(Parameters.GroupsName)
    ...
    MaxTrial=24; if(MaxTrial<10),disp(['WARNING: nb trials ' num2str(MaxTrial)]);end
    ...
    switch indT
        case 1
            save([Directory 'mat\MARTI_fisher_ws_' datestr(now,30) '.mat'],'R')
        case 2
...
toPlot = {'method_mean','users','TestSetRatio'};
for inpName=toPlot
...
Res=[]
for indPl=1:3 %players
    for testR=1:3 %training set ratio
        [indicesP, namesP, cvec]=struc2res(Pall,Param,{1:4,indPl,testR})
    ...
    subplot(1,3,indPl)
    errorbar(repmat([5 15 25]',1,4),cellfun(@mean,AUC'),cellfun(@mean,STD')/3);legend(Param.method_mean,'location','south')
    xlabel('training set (%)')
    ylabel('AUC')
...
set(gcf, 'PaperPosition', [0 0 30 12.5],'units','normalized','outerposition',[0.4 0.1 0.5 .9])
