...
BImap={[1,2,5,3,6,12,14,16,21,22,23,24,25,27,28,29],... %1 full BI mapping (16)
    [5,3,6,12,14,16,21,22,23,24,25,27,28,29],... %2 remove Fp1,Fp2 (14)
    [5,6,12,14,16,21,22,23,24,25,27,28,29],... %3 remove Fp1,Fp2, AFz (13)
    [5,6,13,14,15,21,22,23,24,25,27,28,29],...%4 remove Fp1,Fp2, AFz, T7,T8 add C3,C4(13)
    [5,6,14,21,22,23,24,25,27,28,29],...%5 remove Fp1,Fp2, AFz, T7,T8 (11)
    [5,6,13,14,15,22,23,24,27,28,29],... %6 remove Fp1,Fp2, AFz, T7,T8,P7,P8, add C3 C4 (11)
    [5,6,14,22,23,24,27,28,29],... %7 remove Fp1,Fp2, AFz, T7,T8,P7,P8 (9)
    [22,23,24,26,27,28,29,30],...%8 just P3,Pz,P4,PO7,01,0z,O2,P08 (8)
    [22,23,24,27,28,29]};%9 just P3,Pz,P4,01,0z,O2 (6)
...
% courbe roc 
scores{i,StatsIND,MAP,Trial} = -diff(distances')';
 [TPR,FPR,TH] =roc(Ytest',scores{i,StatsIND,MAP,Trial}');

% Area under curve (higer is better, max=1);
[PerfX,PerfY,~,AUC{i,StatsIND,MAP,Trial},OPTROCPT] = perfcurve(Ytest',scores{i,StatsIND,MAP,Trial}',1);
AUC{i,StatsIND,MAP,Trial}
...
AUCn=cell2mat(AUC(selectedGroups,:,:,:));
%% Analyse mapping
close all
MEANmapping=squeeze(mean(mean(AUCn,4),1));
