    ...
    for indGroup=1:length(GroupsName)
        ...
        try
            for indPlayer=1:2
                ...
                cfg.t_ftimwin    = 3./cfg.foi;   % length of time window = 0.5 sec (short time fourier)
                % OR
                cfg.width = 3; %number of the period of the wavelet
...
close all
Conditions=107:110;
DirectoryFig=['D:\data\Hyperscanning\MARTI\Groups\mat\COH5\']
errorgroups=[];
for indGroup=1:length(GroupsName)
...
for indF=4:3:length(COHall.freq)%:length(COHall.freq)
    ...
    cfg.tail             = 0;
    cfg.clustertail      = 0;
    cfg.alpha            = 0.05; % should be 0.025 for two-sided
...
save([DirectoryFig 'stat.mat'],'stats12','stats34')
...
load([Directory 'MARTI_GroupsName.mat'])
LABELS=importdata([Directory 'MARTImapping.txt']);
load([DirectoryFig 'stat.mat'],'stats12')
stat=stats12
for indF=4:3:58
    try
        measure='Pcoh';
        ...
        cfg.saveaspng=[DirectoryFig 'cluster\' measure '_12_alpha_' num2str(cfg.alpha*100)  '_f' num2str(round(stat{indF}.freq))   ];
