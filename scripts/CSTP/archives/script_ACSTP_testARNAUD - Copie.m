...
clear FILTER
%load data
for indUser=2%:length(users) %select the subject
    ...
    f1=0.25; %(8) low cutoff freq  (bandpass)
    ...
    Delays=6; %(12) +/- nb shifted samples allowed for the jitter correction
    ...
    winElec=findElectrodes(EEG.ElectrodesName,{'P3','P4','O1','O2','F7','F8','T3','T4','T5','T6','T5''','T6''','O1''','O2''','P3"','P4"','PZ"','OZ','I','CB1"','CB2"','CB1','CB2'})
    ...
    ACSTPoptions.SubspaceDim=[31:-1:10];
...
close all
for indSubject=1:length(FILTER)
    Scale=10 %init scale
    for classIND=1:2
...
xlabel('Weights','fontsize',FontSize,'fontname','times new roman','FontAngle','italic')
    saveas(hfighisto,['D:\Mes Documents GIPSA\MATLAB\figures\GONOGO\final\EAvsCSTP\' nameSave '_Subject_' num2str(indSubject) '_Latencies_Weights.tiff'])
saveas(hfighisto,['D:\Mes Documents GIPSA\MATLAB\figures\GONOGO\final\EAvsCSTP\' nameSave '_Subject_' num2str(indSubject) '_Latencies_Weights.fig'])
...
%spaceplots
saveas(gca,['D:\Mes Documents GIPSA\MATLAB\figures\CSPT\GONOGO\CSTP_components\' nameSave '_Subject_' num2str(indSubject) '_components_Pz' num2str((FILTER(indSubject).BestPz(classIND))) '.tiff'])
saveas(gca,['D:\Mes Documents GIPSA\MATLAB\figures\CSPT\GONOGO\CSTP_components\' nameSave '_Subject_' num2str(indSubject) '_components_Pz' num2str((FILTER(indSubject).BestPz(classIND))) '.fig'])
