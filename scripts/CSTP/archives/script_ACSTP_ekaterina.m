...
% --------
% *** 2015-02-18
% Created by L. KORCZOWSKI @GIPSA-Lab, based on work of:
% M. CONGEDO, L. KORCZOWSKI, A. DELORME, F. LOPES DA SILVA "Spatio-Temporal
% Common Pattern a Reference Companion Method for ERP Analysis"
% (to be submitted)
%
%   related functions :
...
close all
clear all
%Directory= '.\LK TOOLBOX\'; %change path if needed
Directory='D:\data\Hyperscanning\BI-multiplayers\Groups\'
%load( [Directory 'ss5_ERWAN_RAW.mat'])
...
% 
Subjects=1 %indice of the subject in 'data' (if several)
%%prepare data
Session=1;
DATAtype=2;%1 for ERWAN 2 for Ekatarina
if DATAtype==1
%for erwan data
...
%user parameters for the ACSTP to improve converge :
    winElec=[7,10,11,12,13,14,15,16]; %(13)* electrodes used for latency calculation and Pz selection
    winTime=[5:80]; %(14)* time window used for latency calculation and Pz selection
...
maxIter_Latcor=10;
Pzfs=[size(Xall,1):-floor(size(Xall,1)/10):10];%choose the screening of the Pz
%Y=ones(size(Y));
...
for Pzf=Pzfs;
    ...
    [Bs Bt As At Class eigV]=ACSTP(X,Y,Xbarz,Weights,winElec,winTime,Pzf,Xnoise,Ynoise);
...
for IND=1:2
    ...
    winTime2=5:60;
    [Pzind SNR]=best_Pz(Save{2},winElec,winTime);
    [Bs]=Save{3}{Pzind}{1};
    [Bt]=Save{3}{Pzind}{2};
    [As]=Save{3}{Pzind}{3};
    [At]=Save{3}{Pzind}{4};
