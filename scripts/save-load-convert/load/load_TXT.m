...
fs=128;
formatSpec = '%s';
[Text]=fopen([FULLDIR '\EEGnotch.txt'])
...
if mod==1
EEGtxt{userNB}.session{k}.phase{phase+1}.online=reshape(E,16,length(E)/16);
end
if mod==2
    EEGtxt{userNB}.session{k}.phase{phase+1}.training=reshape(E,16,length(E)/16);
