...
% this script tests the identification method making use non-gaussianity of the residuals (approach 2)

clear; close all; clc;
nfft=512; % number of frequency bins
fc=1; % sample frequency
N=1000; % length of simulated series
...
U=InstModelfilter(N,Sw,'ExtendedNonGauss',B0); % nongaussian innovations W, U determined from B0
...
[eBm,eB0,eSw,eAm,eSu,eU,eW,percup,ki]=idMVAR0ng(Y,p,idMode);
...
% trick: for the surrogates I use prior identification method exploiting the  causal ordering estimated by ng identification
% the problem is that  ng identification easily fails on the surrogates because they may not meet identification requirements (estimated causal ordering may be misleading, confusing also lagged coeffs and giving too high thresholds...)
if do_surrogates=='y'
    eCOHs=NaN*ones(M,M,nfft,numsurr); ePCOHs=eCOHs; eEDCs=eCOHs; eEPDCs=eCOHs; eLDCs=eCOHs; eLPDCs=eCOHs;
    for is=1:numsurr 
        clc; disp(['surrogate ' int2str(is) ' of ' int2str(numsurr) ', FT surrogates']);
        Ys=surrVFT(Y); % FT surrogates
        [eBms,eB0s,eSws]=idMVAR0prior(Ys,p,ki,idMode); % USE ESTIMATED CAUSAL ORDER and PRIOR IDENTIFICATION!
        ...
        for ii=1:M
            for jj=1:M
                if ii~=jj
                    clc; disp(['surrogate ' int2str(is) ' of ' int2str(numsurr) ', ' int2str(ii) '->' int2str(jj) ', CFTf surrogates' ]);
                    ...
                    clc; disp(['surrogate ' int2str(is) ' of ' int2str(numsurr) ', ' int2str(ii) '->' int2str(jj) ', CFTd surrogates']);
